...
parseLine[line_String] := Module[{parts, kinds, bparams, sConn, iConn, eConn, sp, ip, ep,
    ...
    (* Debug: print parsed SUGRA info *)
    If[anomVal =!= None || sigVal =!= None || detVal =!= None,
        Print["DEBUG SUGRA: anom=", anomVal, " sig=", sigVal, " det=", detVal];
    ];
        ...
        Which[
            position == "right",
                ...
                If[idx <= Length[sortedS] && rightParam == 0, 
                    rightParam = sortedS[[idx]]; rightExt = Lookup[sideLinkExternals, sortedS[[idx]], <||>]; idx++];
                If[idx <= Length[sortedS] && topParam == 0,
                    topParam = sortedS[[idx]]; topExt = Lookup[sideLinkExternals, sortedS[[idx]], <||>]; idx++];
                If[idx <= Length[sortedS] && bottomParam == 0,
                    bottomParam = sortedS[[idx]]; bottomExt = Lookup[sideLinkExternals, sortedS[[idx]], <||>]; idx++],
            position == "middle",
            ...
            position == "single",
                ...
                If[idx <= Length[sortedS] && leftParam == 0,
                    leftParam = sortedS[[idx]]; leftExt = Lookup[sideLinkExternals, sortedS[[idx]], <||>]; idx++];
                If[idx <= Length[sortedS] && topParam == 0,
                    topParam = sortedS[[idx]]; topExt = Lookup[sideLinkExternals, sortedS[[idx]], <||>]; idx++];
                If[idx <= Length[sortedS] && rightParam == 0,
                    rightParam = sortedS[[idx]]; rightExt = Lookup[sideLinkExternals, sortedS[[idx]], <||>]; idx++];
            ...
            With[{sig = group[[1, 2]], 
                ...
                For[j = 1, j <= Length[items], j++,
                    With[{latexItem = items[[j, 1]], sugraItem = items[[j, 3]]},
                        (* Debug *)
                        Print["DEBUG OUTPUT: sugraItem=", sugraItem, " anom=", sugraItem["anom"], " test=", sugraItem["anom"] =!= None];
                        ...
                        If[includeSugraInfo && sugraItem["anom"] =!= None,
                            ...
                            If[sugraItem["det"] =!= None,
                                infoStr = infoStr <> ", \\det=" <> ToString[sugraItem["det"]];
                            ];
                            infoStr = infoStr <> ")";
                        ];
