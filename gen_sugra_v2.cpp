...
// v2: Node-aware LaTeX rendering (⊗ notation for Nk entries)
//
// Algorithm:
...
//      v2: Nk nodes rendered as ⊗ with gauge algebra
//
// Compile:
//   g++ -std=c++17 -O2 -o gen_sugra_v2 gen_sugra_v2.cpp Tensor.C \
...
// ============================================================================
// v2: Block-level topology info for Nk entries
// ============================================================================
// Maps each curve index to its parent block, enabling:
//   - Node curves → gauge algebra rendering
//   - Interior link curves → compressed ⊗ rendering

enum class BlockType { Node, InteriorLink, SideLink, Instanton, External, Unknown };

struct BlockInfo {
    BlockType type;
    int param;           // block param (node gauge param, link AL param, etc.)
    int start, end;      // curve index range [start, end)
};

struct TopoBlockMap {
    std::vector<BlockInfo> blocks;
    std::vector<int> curve_to_block;  // curve_idx → block index
    
    bool is_node(int curve_idx) const {
        if (curve_idx < 0 || curve_idx >= (int)curve_to_block.size()) return false;
        return blocks[curve_to_block[curve_idx]].type == BlockType::Node;
    }
    bool is_interior_link(int curve_idx) const {
        if (curve_idx < 0 || curve_idx >= (int)curve_to_block.size()) return false;
        return blocks[curve_to_block[curve_idx]].type == BlockType::InteriorLink;
    }
    const BlockInfo& block_of(int curve_idx) const {
        return blocks[curve_to_block[curve_idx]];
    }
    int block_id(int curve_idx) const {
        if (curve_idx < 0 || curve_idx >= (int)curve_to_block.size()) return -1;
        return curve_to_block[curve_idx];
    }
};
...
TopoBlockMap get_block_map(const CatalogEntry& entry) {
    ...
    int offset = 0;
    auto add_block = [&](BlockType type, int param, Kind kind, int spec_param) {
        int ncurves = getCurveCount(spec_param, kind);
        result.blocks.push_back({type, param, offset, offset + ncurves});
        offset += ncurves;
    };
    
    // Blocks (nodes + interior links)
    for (auto& blk : topo.block) {
        Spec sp = nk_detail::make_spec(blk.kind, blk.param);
        BlockType bt = (blk.kind == LKind::g) ? BlockType::Node : BlockType::InteriorLink;
        add_block(bt, blk.param, sp.kind, sp.param);
    }
    // Sidelinks
    for (auto& sl : topo.side_links) {
        add_block(BlockType::SideLink, sl.param, Kind::SideLink, sl.param);
    }
    // Instantons
    for (auto& inst : topo.instantons) {
        add_block(BlockType::Instanton, inst.param, Kind::SideLink, inst.param);
    }
    // Externals (from catalog, not SUGRA externals)
    for (auto& ext : topo.externals) {
        add_block(BlockType::External, ext.param, Kind::External, ext.param);
    }
    
    // Build curve_to_block map
    int total = offset;
    result.curve_to_block.resize(total, -1);
...
// Gauge algebra LaTeX string from node param
...
// Interior link ⊗ label from param
std::string interior_link_label(int param) {
    std::string s = std::to_string(param);
    if (s == "331") return "\\overset{3,3}{\\bigcirc}";  // special case
    if (s.length() >= 2) {
        std::string digits;
...
std::string if_to_latex(const Eigen::MatrixXi& IF, int ext_idx = -1,
    ...
    bool has_blocks = !bmap.blocks.empty();  // block map covers base curves; external curve handled separately
    ...
    // Find endpoint: degree 1, prefer not external
    int start = -1;
    for (int i = 0; i < n; i++)
    ...
    // Helper: label for a single curve (non-block mode / sidelink curves)
    ...
    // Mark entire block as visited, return unvisited neighbors outside block
...
int main(int argc, char** argv) {
    ...
    // v2: Pre-compute block maps for all Nk entries
    std::map<int, TopoBlockMap> catalog_block_maps;  // catalog_id → block map
    ...
    // v2: Helper to get block map for a catalog entry
    ...
    // ─── Non-unimodular: split by ext type, show T_max ───
    {
        ...
        auto write_nonuni_file = [&](const std::string& filename,
            ...
            for (auto& e : entries) {
                ...
                if (bk != cur_base_key) {
                    cur_base_key = bk;
                    tex << "\\smallskip{\\small\\textbf{Base:} $" << if_to_latex(r.base_IF, -2, get_blocks(r.catalog_id)) << "$}\\smallskip\n\n";
                }
                int delta = r.anomaly.H_charged - r.anomaly.V + 29 * r.anomaly.T - 273;
                int int_num = r.externals.empty() ? 1 : r.externals[0].int_num;
                tex << "$" << if_to_latex(r.final_IF, -1, get_blocks(r.catalog_id)) << "$";
