...
// Central charge at level k: c(k) = k * dim / (k + h∨)
// k = intersection number between -1 curve and the neighbor curve
// E8 budget = c(E8, level 1) = 248/31 = 8
...
const PortGaugeInfo GAUGE_SP1   = {"sp1",     3,  2};    // c(k=1) = 1.0
const PortGaugeInfo GAUGE_SU2   = {"su2",     3,  2};    // c(k=1) = 1.0
const PortGaugeInfo GAUGE_SU3   = {"su3",     8,  3};    // c(k=1) = 2.0
const PortGaugeInfo GAUGE_G2    = {"g2",     14,  4};    // c(k=1) = 2.8
const PortGaugeInfo GAUGE_SO7   = {"so7",    21,  5};    // c(k=1) = 3.5
const PortGaugeInfo GAUGE_SO8   = {"so8",    28,  6};    // c(k=1) = 4.0
const PortGaugeInfo GAUGE_SO9   = {"so9",    36,  7};    // c(k=1) = 4.5
const PortGaugeInfo GAUGE_SO10  = {"so10",   45,  8};    // c(k=1) = 5.0
const PortGaugeInfo GAUGE_SO11  = {"so11",   55,  9};    // c(k=1) = 5.5
const PortGaugeInfo GAUGE_SO12  = {"so12",   66, 10};    // c(k=1) = 6.0
const PortGaugeInfo GAUGE_F4    = {"f4",     52,  9};    // c(k=1) = 5.2
const PortGaugeInfo GAUGE_E6    = {"e6",     78, 12};    // c(k=1) = 6.0
const PortGaugeInfo GAUGE_E7    = {"e7",    133, 18};    // c(k=1) = 7.0
const PortGaugeInfo GAUGE_E8    = {"e8",    248, 30};    // c(k=1) = 8.0
...
bool check_layer2_e8_constraint(const Eigen::MatrixXi& IF,
    ...
    for (int m1 : minus1_curves) {
        // Collect all non-(-1) neighbors, their gauges, and intersection numbers
        ...
        // Check (b): central charge budget  sum c_g(k) <= 8
...
// ── A/D type LST spectrum filter ──
// Build spectra of dummy A-type (1-2-...-2-1) and D-type LSTs,
// then remove any entries that match.

std::set<std::string> build_AD_spectra(int max_n = 25) {
    std::set<std::string> ad;
    
    auto spec_key = [](const Eigen::MatrixXi& IF) -> std::string {
    ...
    // A-type: 1-2-...-2-1 (n curves)
    for (int n = 1; n <= max_n; n++) {
        Eigen::MatrixXi IF = Eigen::MatrixXi::Zero(n, n);
        IF(0,0) = -1; IF(n-1,n-1) = -1;
        for (int i = 1; i < n-1; i++) IF(i,i) = -2;
        for (int i = 0; i < n-1; i++) { IF(i,i+1) = 1; IF(i+1,i) = 1; }
        ad.insert(spec_key(IF));
    }
    
    // D-type: D(3)=212, D(n>=4)=2{2,2}2...21
    for (int n = 3; n <= max_n; n++) {
        Eigen::MatrixXi IF;
        if (n == 3) {
            IF = Eigen::MatrixXi::Zero(3,3);
            IF(0,0) = -2; IF(1,1) = -1; IF(2,2) = -2;
            IF(0,1) = 1; IF(1,0) = 1; IF(1,2) = 1; IF(2,1) = 1;
        } else {
            IF = Eigen::MatrixXi::Zero(n,n);
            for (int i = 0; i <= n-4; i++) IF(i,i) = -2;
            IF(n-3,n-3) = -1; IF(n-2,n-2) = -2; IF(n-1,n-1) = -2;
...
bool is_AD_type(const std::vector<double>& spectrum, const std::set<std::string>& ad_spectra) {
    std::ostringstream ss;
    ss << std::fixed << std::setprecision(8);
    ...
    return ad_spectra.count(ss.str()) > 0;
}

template<typename T>
std::vector<T> remove_AD_types(std::vector<T>& results, const std::set<std::string>& ad_spectra) {
    std::vector<T> filtered;
    for (auto& r : results) {
        r.compute_spectrum();
        if (!is_AD_type(r.spectrum, ad_spectra)) {
...
int main(int argc, char** argv) {
    ...
    if (catalog_mode) {
        ...
        // Remove A/D type LSTs
        auto ad_spectra = build_AD_spectra();
        auto filtered_results = remove_AD_types(unique_results, ad_spectra);
        std::cout << "Removed " << (unique_results.size() - filtered_results.size()) << " A/D type LSTs\n";
        std::cout << "Final count: " << filtered_results.size() << "\n";
        
        // NHC stats
        if (nhc_stats || verbose) {
            print_nhc_statistics(filtered_results);
        }
        
        // Write catalog
        write_catalog(catalog_filename, filtered_results, nhc_filter);
    ...
    // Remove A/D type LSTs
    auto ad_spectra = build_AD_spectra();
    unique_results = remove_AD_types(unique_results, ad_spectra);
    std::cout << "Removed A/D types, final count: " << unique_results.size() << "\n\n";
